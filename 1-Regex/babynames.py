...
def extractFemaleNames(filename,letter):
  ...
  for line in file:
    tuples=re.findall(r'<td>(\w+)</td><td>(\w+)</td><td>('+re.escape(letter)+r'\w+)'+r'</td>',line)
    if tuples:
      s=tuples[0][2]+' '+tuples[0][0]
      ls.append(s)
...
def extractMaleNames(filename,letter):
  ...
  file=open(filename,'r')
  for line in file:
    tuples=re.findall(r'<td>(\w+)</td><td>('+re.escape(letter)+r'\w+)'+r'</td><td>(\w+)</td>',line)
    if tuples:
      s=tuples[0][1]+' '+tuples[0][0]
      ls.append(s)
...
def extractDocAndPopularityDate(filename):
  ...
  file=open(filename,'r')
  for line in file:
    date=re.findall(r'</h1>(\w+\s\w+\S\W+[0-9]+)',line)
    popularity=re.findall(r'(Popularity\s\w+\s+[0-9]+)</h3>',line)
   
    if date:
      s=str(date)
    if popularity  :
      s=s+' and '+str(popularity)
